  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7b6d4f] transition-colors hover:text-[#8a6500]"
  ...
  if (isLoading) {
    return (
      <div className="space-y-6 p-6">
        <Skeleton className="h-10 w-48 bg-[#e4dac9]" />
        <Skeleton className="h-40 w-full bg-[#e4dac9]" />
        <Skeleton className="h-64 w-full bg-[#e4dac9]" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 p-6">
      <header>
        ...
        <p className="text-[#6a5f49]">Edit your personal information</p>
      </header>

      {/* Profile Card */}
      <Card className="border-[#8a6500]/30 bg-[#f4ece0]/85 p-6">
        <form onSubmit={onProfileSubmit} className="flex flex-col gap-8 md:flex-row">
          ...
          <div className="relative group self-center md:self-start">
            <div className="relative flex h-32 w-32 items-center justify-center overflow-hidden rounded-full border-2 border-[#8a6500]/30 bg-[#ece2d1]">
              {currentAvatar ? (
                <img src={currentAvatar} alt="Profile" className="h-full w-full object-cover" />
              ) : (
                <span className="text-3xl font-bold text-[#6b4f02]">{getInitials(data?.name)}</span>
              )}

              <label htmlFor="avatar" className="absolute inset-0 flex cursor-pointer items-center justify-center bg-black/30 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          <div className="flex-1 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-2xl font-semibold text-[#2f2615]">{data?.name}</h3>
                <p className="text-[#6a5f49]">@{data?.role || "user"}</p>
              </div>
      ...
      {/* Password Card */}
      <Card className="border-[#8a6500]/30 bg-[#f4ece0]/85 p-6">
        <h3 className="mb-6 text-2xl font-semibold text-[#2f2615]">Change password</h3>
