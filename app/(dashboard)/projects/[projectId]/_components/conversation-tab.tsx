...
export function ConversationTab({
  ...
  return (
    <Card className="border-[#8a6500]/25 bg-[#f4ece0]/85 p-4">
      <h3 className="text-title-24">
        Approve updated bathroom tile layout for ensuite
      </h3>
      <p className="text-body-16 mb-5 mt-2 text-[#6a5f49]">
      ...
      <div className="space-y-4">
        {messages.length === 0 ? (
          <p className="text-body-16 text-[#6a5f49]">No messages yet.</p>
        ) : (
          messages.map((item) => (
            <div key={item._id}>
              <p className="text-body-16 font-semibold text-[#3d331f]">
                {item.sender?.name || "Unknown"}
              </p>
              <p className="text-body-16 text-[#4f4638]">{item.message || "-"}</p>
            </div>
          ))
        )}
      </div>
